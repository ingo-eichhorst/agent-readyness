...
must_haves:
  truths:
    - "ars scan completes on a Python-only project directory and discovers .py files"
    - "ars scan completes on a TypeScript-only project directory and discovers .ts/.tsx files"
    - "ars scan on a polyglot repo discovers files from all three languages (Go, Python, TypeScript)"
    - "Tree-sitter parses Python source files and produces queryable syntax trees"
    - "Tree-sitter parses TypeScript source files and produces queryable syntax trees"
    - "ars scan completes without memory growth from Tree-sitter (all parsers and trees closed)"
  artifacts:
...
<task type="auto">
  <name>Task 1: Extend discovery walker, classifier, and types for multi-language</name>
  <files>
    internal/discovery/walker.go
    internal/discovery/classifier.go
    internal/discovery/walker_test.go
...
5. Update tests:
   ...
   - Add walker tests that discover files in valid-python-project and valid-ts-project (fixtures created in Task 2)
...
`cd /Users/ingo/agent-readyness && go test ./internal/discovery/... -v` -- all tests pass including new multi-language tests
  ...
  <done>Walker discovers .py, .ts, .tsx files. Classifier handles Python/TypeScript test patterns. Language detection works. All existing Go discovery tests still pass.</done>
</task>

<task type="auto">
  <name>Task 2: Create test fixtures and integrate Tree-sitter parser</name>
  <files>
    testdata/valid-python-project/app.py
    testdata/valid-python-project/test_app.py
    testdata/valid-python-project/pyproject.toml
...
3. Create `internal/parser/treesitter.go`:
...
5. Write tests in `internal/parser/treesitter_test.go`:
...
`cd /Users/ingo/agent-readyness && CGO_ENABLED=1 go test ./internal/parser/... -v` -- all tests pass including new Tree-sitter tests. Also: `CGO_ENABLED=1 go build ./...` succeeds.
  ...
  <done>Test fixtures exist for Python, TypeScript, and polyglot projects. TreeSitterParser exists with Python, TypeScript, TSX support. Parsers pool correctly. Memory management via explicit Close(). Tests verify parsing of real Python/TypeScript files.</done>
</task>
