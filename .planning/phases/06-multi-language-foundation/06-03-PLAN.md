...
must_haves:
  truths:
    ...
    - "C2 Go analyzer produces a scored C2 category result in unit tests"
...
Output: Working C2 Go analyzer, map-based ScoringConfig, C2 scoring with breakpoints. NOTE: C2 is NOT wired into the pipeline in this plan -- that happens in Plan 04. This plan is verified via unit tests only.
</objective>
...
<task type="auto">
  <name>Task 2: Implement C2 Go analyzer (unit-tested, not yet wired into pipeline)</name>
  <files>
    internal/analyzer/c2_semantics.go
    internal/analyzer/c2_go.go
    internal/analyzer/c2_go_test.go
...
1. Create `internal/analyzer/c2_semantics.go`:
   ...
   - NOTE: Do NOT register C2Analyzer in pipeline.go in this plan. Pipeline registration happens in Plan 04 Task 2 alongside the multi-language wiring.
...
3. Write tests `internal/analyzer/c2_go_test.go`:
   - Test C2GoAnalyzer directly by constructing it with parsed packages from the project's own codebase
   - Verify C2LanguageMetrics fields are populated (TypeAnnotationCoverage == 100, NamingConsistency > 0, MagicNumberRatio >= 0)
   - Test C2Analyzer.Analyze() with a Go AnalysisTarget and verify it returns Category: "C2" with populated metrics
   - Test edge cases: empty interface{} detection, any keyword detection
  </action>
  <verify>
`cd /Users/ingo/agent-readyness && go test ./internal/analyzer/... -v -run C2` -- C2 analyzer tests pass. Then `go test ./... 2>&1` -- all tests pass. NOTE: Do NOT test `ars scan` for C2 output here -- C2 is not registered in the pipeline yet (that is Plan 04).
  </verify>
  <done>C2 Go analyzer produces metrics for interface{}/any usage, naming consistency, magic numbers, and nil safety. C2Analyzer dispatches to Go implementation. Unit tests verify real Go code analysis. C2Analyzer is ready to be registered in the pipeline (Plan 04).</done>
</task>
...
4. C2 Go analyzer produces correct metrics in unit tests
...
<success_criteria>
- C2 Go analysis produces meaningful metrics for Go codebases (verified via unit tests)
- ScoringConfig is map-based and extensible for C4/C5/C7
- All existing scoring tests pass with identical results
- C2Analyzer is ready for pipeline registration in Plan 04
</success_criteria>
