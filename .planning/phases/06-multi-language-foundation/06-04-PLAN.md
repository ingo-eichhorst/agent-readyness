...
must_haves:
  truths:
    ...
    - "ars scan auto-detects project languages without requiring --lang flag or any language-specific arguments"
  artifacts:
  ...
  key_links:
    ...
    - from: "internal/pipeline/pipeline.go"
      to: "internal/analyzer/c2_semantics.go"
      via: "Pipeline registers C2Analyzer in analyzer list"
      pattern: "C2Analyzer"
    - from: "internal/pipeline/pipeline.go"
...
<task type="auto">
  <name>Task 1: Implement C2 Python analyzer</name>
  <files>
    internal/analyzer/c2_python.go
    internal/analyzer/c2_python_test.go
  </files>
  <action>
Create `internal/analyzer/c2_python.go` -- `C2PythonAnalyzer`:
...
Write tests `c2_python_test.go`: test against testdata/valid-python-project. Verify type annotation coverage > 0, naming consistency > 0, magic number detection works.
  </action>
  <verify>
`cd /Users/ingo/agent-readyness && CGO_ENABLED=1 go test ./internal/analyzer/... -v -run C2Python` -- Python C2 tests pass
  </verify>
  <done>C2 Python analyzer measures type annotations, naming (PEP 8), magic numbers, and mypy/pyright config. Tests verify analysis of real Python test fixture.</done>
</task>

<task type="auto">
  <name>Task 2: Implement C2 TypeScript analyzer and update C2 dispatcher</name>
  <files>
    internal/analyzer/c2_typescript.go
    internal/analyzer/c2_typescript_test.go
    internal/analyzer/c2_semantics.go
  ...
  <action>
1. Create `internal/analyzer/c2_typescript.go` -- `C2TypeScriptAnalyzer`:
   - Accepts a `*parser.TreeSitterParser` in constructor
   - `Analyze(target *types.AnalysisTarget) (*types.C2LanguageMetrics, error)`
...
2. Update `internal/analyzer/c2_semantics.go`:
...
3. Write tests `c2_typescript_test.go`: test against testdata/valid-ts-project. Verify type coverage > 0, strict mode detected from tsconfig.json, magic numbers found.
  </action>
  <verify>
`cd /Users/ingo/agent-readyness && CGO_ENABLED=1 go test ./internal/analyzer/... -v -run "C2TypeScript|C2Semantics"` -- TypeScript C2 tests and full C2 dispatcher tests pass
  </verify>
  <done>C2 TypeScript analyzer measures type coverage, tsconfig strict, magic numbers, null safety. C2Analyzer dispatches to all three languages (Go, Python, TypeScript) and aggregates results.</done>
</task>

<task type="auto">
  <name>Task 3: Config system, pipeline wiring, CLI, and output updates</name>
  <files>
    internal/config/config.go
    internal/config/config_test.go
    internal/pipeline/pipeline.go
...
4. Update `cmd/scan.go`:
   ...
   - The pipeline auto-detects project languages from discovered files. NO --lang flag needed. User runs `ars scan <dir>` and languages are detected automatically.
...
- `/tmp/ars scan ./testdata/valid-python-project` -- shows C2 scores for Python (auto-detected, no --lang flag)
- `/tmp/ars scan ./testdata/valid-ts-project` -- shows C2 scores for TypeScript (auto-detected, no --lang flag)
...
CLI-04 auto-detection verification:
- `/tmp/ars scan ./testdata/valid-python-project` works WITHOUT any `--lang python` flag -- language is auto-detected from file extensions and project markers (pyproject.toml)
- `/tmp/ars scan ./testdata/valid-ts-project` works WITHOUT any `--lang typescript` flag -- language is auto-detected from tsconfig.json and .ts files
  </verify>
  <done>Config system loads .arsrc.yml with validation. Pipeline registers C2Analyzer and runs multi-language analysis. CLI accepts Python/TypeScript projects with automatic language detection (no --lang flag needed). Output shows C2 scores and per-language details. All phase 6 success criteria met.</done>
</task>
...
3. `ars scan` on Python project shows C2 scores (auto-detected language)
4. `ars scan` on TypeScript project shows C2 scores (auto-detected language)
...
9. No `--lang` flag needed -- language auto-detection works from file extensions and project markers
</verification>
...
- Phase 6 success criteria 6: Language auto-detection works without user flags (CLI-04)
