...
6. Create internal/llm/client_test.go with mock Anthropic client tests:
   - TestNewClient_MissingAPIKey: returns error when API key empty
   - TestEvaluateContent_Success: mock client returns valid JSON, verify parsing
   - TestEvaluateContent_InvalidJSON: mock client returns malformed response, verify error handling
   - TestEvaluateContent_RateLimit: mock 429 response, verify retry behavior
   - Use httptest.NewServer to mock Anthropic API responses
  </action>
  <verify>
go build ./internal/llm/... compiles without errors
go test ./internal/llm/... passes (mock tests verify client behavior without real API calls)
  </verify>
  <done>
LLM client package exists with Anthropic SDK integration, cost estimation, evaluation prompts, and mock tests verifying client behavior.
  </done>
</task>
