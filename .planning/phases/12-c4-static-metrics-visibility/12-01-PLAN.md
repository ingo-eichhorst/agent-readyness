...
1. Add an early return check for availability (matching C5/C7 pattern):
```go
if !m.Available {
...
`grep -n "colorForIntInverse" internal/output/terminal.go` shows helper function.
  </verify>
  <done>renderC4 displays static metrics always, LLM metrics as N/A when disabled</done>
</task>
