...
must_haves:
  ...
  artifacts:
    ...
    - path: "internal/agent/cli_test.go"
      provides: "Tests for CLI detection"
...
<task type="auto">
  ...
  <files>internal/agent/cli.go, internal/agent/cli_test.go</files>
  <action>
...
Create `internal/agent/cli_test.go` with:
...
Run `go test ./internal/agent/... -run TestDetect`
  ...
  <done>CLIStatus struct and DetectCLI function exist, cli_test.go exists with tests, tests pass</done>
</task>
...
1. Add import `"github.com/ingo/agent-readyness/internal/agent"`
...
6. **Keep llm import for now** - the C4 analyzer still compiles with the old import present.
   Plan 02 will delete the entire internal/llm package and this import will be removed as part of that cleanup.
   This avoids a temporary compilation break between plans.
...
Manual test: `go run . scan . --json` should work (LLM features disabled until pipeline integration in plan 02)
  ...
  <done>C4 analyzer compiles and uses agent.Evaluator, existing tests pass, llm import kept temporarily for compilation</done>
</task>
...
4. New files exist: cli.go, cli_test.go, evaluator.go, prompts.go in internal/agent/
