...
must_haves:
  truths:
    ...
    - "C7 scoring uses CLI evaluator instead of SDK"
...
<task type="auto">
  <name>Task 2: Update scan command flags and flow</name>
  <files>cmd/scan.go</files>
  <action>
Update `cmd/scan.go` for new flag structure:

1. Remove flag variables:
   - Delete `enableC4LLM bool`
   - Keep `enableC7 bool` (C7 requires explicit opt-in due to cost/time)
...
4. Update RunE function:
   - Remove entire `if enableC4LLM { ... }` block (ANTHROPIC_API_KEY check, cost estimate, confirmation)
   - Remove llmClient variable and all references

5. After pipeline creation, check CLI status and show info:
   ```go
...
6. Update C7 handling:
   - Remove ANTHROPIC_API_KEY requirement
   - Keep CLI availability check (`agent.CheckClaudeCLI()`)
   - Remove cost estimate and user confirmation (CLI handles auth/billing)
   - Simplify to just: if enableC7, check CLI, call `p.SetC7Enabled()`

7. Remove imports:
   - Remove `"github.com/ingo/agent-readyness/internal/llm"`
   - Remove `"bufio"` if only used for LLM confirmation
  </action>
  <verify>
Run: `go build ./cmd/...` - should compile
Run: `go run . scan --help` - should show --no-llm flag, NOT show --enable-c4-llm
  </verify>
  <done>Scan command has --no-llm flag, no --enable-c4-llm, no ANTHROPIC_API_KEY requirement</done>
</task>

<task type="auto">
  <name>Task 3: Migrate C7 scorer and remove SDK dependency</name>
...
2. Update Score method:
   ...
   - EvaluationResult has same Score/Reason fields, so mapping is direct
...
Run: `go test ./internal/analyzer/...` - C7 tests pass
  ...
  <done>SDK removed, scorer uses CLI evaluator, C7 analyzer uses CLI evaluator, go.mod clean</done>
</task>
