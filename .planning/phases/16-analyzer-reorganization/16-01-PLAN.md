...
must_haves:
  truths:
    - "Shared tree-sitter helper functions are exported and accessible from subpackages"
    - "Type aliases exist for all C1-C7 analyzers"
    - "Constructor wrappers exist for all NewCxAnalyzer functions"
    - "Category subdirectories exist with correct naming"
  artifacts:
    ...
    - path: "internal/analyzer/c1_code_quality/"
      provides: "Directory for C1 analyzer files"
    - path: "internal/analyzer/c7_agent/"
      provides: "Directory for C7 analyzer files"
  key_links:
...
<objective>
Create the foundation for analyzer reorganization: subdirectory structure, shared utilities file with exported tree-sitter helpers, and root-level re-export file with type aliases.
...
Output: 7 category subdirectories, shared.go with exported helpers, analyzer.go with type aliases and constructor wrappers.
</objective>
...
<task type="auto">
  <name>Task 2: Create shared.go with exported utilities</name>
  <files>internal/analyzer/shared.go</files>
  <action>
...
After creating shared.go, delete helpers.go (its content is now in shared.go).
  </action>
  <verify>
`go build ./internal/analyzer/...` succeeds (shared.go compiles)
  </verify>
  <done>
shared.go exists with WalkTree, NodeText, CountLines, ImportGraph, and BuildImportGraph exported. helpers.go removed.
  </done>
</task>
...
Note: This file will NOT compile yet because subpackages are empty. That's expected - Plan 02 will move the actual analyzer code into the subdirectories.
  ...
  <verify>
File exists at internal/analyzer/analyzer.go with correct structure (will fail build until Plan 02 completes - this is expected)
  </verify>
  <done>
analyzer.go exists with all 7 type aliases and 7 constructor wrappers. File structure matches RESEARCH.md template.
  </done>
</task>
...
2. shared.go compiles: `go build ./internal/analyzer/shared.go` (as standalone)
3. helpers.go is removed
4. analyzer.go has correct structure (manual inspection - won't compile until Plan 02)
</verification>
...
- No changes to existing analyzer files yet (that's Plan 02)
</success_criteria>
