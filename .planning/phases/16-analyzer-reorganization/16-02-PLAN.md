...
must_haves:
  truths:
    - "All analyzer source files moved from root to category subdirectories"
    - "go build ./... succeeds with no import errors"
    - "go test ./... passes (all 100+ tests)"
    - "pipeline.go still imports analyzer package and calls NewCxAnalyzer unchanged"
  artifacts:
    - path: "internal/analyzer/c1_code_quality/codehealth.go"
      provides: "C1Analyzer type and NewC1Analyzer constructor (moved from c1_codehealth.go)"
      exports: ["C1Analyzer", "NewC1Analyzer"]
    - path: "internal/analyzer/c2_semantics/semantics.go"
      provides: "C2Analyzer type and NewC2Analyzer constructor (moved from c2_semantics.go)"
      exports: ["C2Analyzer", "NewC2Analyzer"]
    - path: "internal/analyzer/c7_agent/agent.go"
      provides: "C7Analyzer type and NewC7Analyzer constructor (moved from c7_agent.go)"
  ...
  key_links:
    ...
    - from: "internal/analyzer/analyzer.go"
      to: "internal/analyzer/c1_code_quality/codehealth.go"
      via: "type alias references C1Analyzer defined in moved file"
      pattern: "type C1Analyzer = c1.C1Analyzer"
...
**Critical understanding:** The existing type definitions STAY in the moved files. For example:
- `c1_codehealth.go` already contains `type C1Analyzer struct {...}` and `func NewC1Analyzer(...)`
- When moved to `c1_code_quality/codehealth.go` with `package c1`, it becomes `c1.C1Analyzer`
- The root `analyzer.go` type alias `type C1Analyzer = c1.C1Analyzer` references THIS type
- **No separate analyzer.go file is created in c1_code_quality/**
...
`go test ./internal/analyzer/c1_code_quality/...` passes
  ...
  <done>
C1 files moved to c1_code_quality/. Package declarations updated to `package c1`. codehealth.go contains C1Analyzer type (accessible as c1.C1Analyzer). Shared utility calls use `analyzer.*` prefix. Original c1_*.go files deleted from root.
  </done>
</task>
...
**C2 File Mapping:**
- c2_semantics.go -> c2_semantics/semantics.go (contains C2Analyzer type)
...
**C3 File Mapping:**
- c3_architecture.go -> c3_architecture/architecture.go (contains C3Analyzer type)
...
**Critical understanding:** Types remain in moved files:
- `c2_semantics.go` contains `type C2Analyzer struct` -> becomes `c2.C2Analyzer` in semantics.go
- `c3_architecture.go` contains `type C3Analyzer struct` -> becomes `c3.C3Analyzer` in architecture.go
- **No separate analyzer.go files created in subdirectories**
...
`go test ./internal/analyzer/c2_semantics/... ./internal/analyzer/c3_architecture/...` passes
  ...
  <done>
C2, C3 files moved to subdirectories. Package declarations updated. semantics.go contains C2Analyzer (c2.C2Analyzer). architecture.go contains C3Analyzer (c3.C3Analyzer). Shared utility calls use `analyzer.*` prefix. Filter functions duplicated in C3. Original c2_*.go and c3_*.go files deleted from root.
  </done>
</task>
...
**File Mapping:**
- c4_documentation.go -> c4_documentation/documentation.go (contains C4Analyzer)
- c4_documentation_test.go -> c4_documentation/documentation_test.go
- c5_temporal.go -> c5_temporal/temporal.go (contains C5Analyzer)
- c5_temporal_test.go -> c5_temporal/temporal_test.go
- c6_testing.go -> c6_testing/testing.go (contains C6Analyzer)
...
- c7_agent.go -> c7_agent/agent.go (contains C7Analyzer)
- c7_agent_test.go -> c7_agent/agent_test.go

**Critical understanding:** Types remain in moved files:
- `c4_documentation.go` -> `c4.C4Analyzer` in documentation.go
- `c5_temporal.go` -> `c5.C5Analyzer` in temporal.go
- `c6_testing.go` -> `c6.C6Analyzer` in testing.go
- `c7_agent.go` -> `c7.C7Analyzer` in agent.go
- **No separate analyzer.go files created in subdirectories**
...
`go test ./internal/analyzer/c4_documentation/... ./internal/analyzer/c5_temporal/... ./internal/analyzer/c6_testing/... ./internal/analyzer/c7_agent/...` passes
  ...
  <done>
C4, C5, C6, C7 files moved to subdirectories. Package declarations updated. Each main file (documentation.go, temporal.go, testing.go, agent.go) contains its respective CxAnalyzer type. C6 shared utility calls use `analyzer.*` prefix. Original c4_*.go, c5_*.go, c6_*.go, c7_*.go files deleted from root.
  </done>
</task>
...
1. Full build: `go build ./...`
   ...
   - Verifies analyzer.go re-exports work (type aliases reference types in moved files)
...
5. Verify wiring is correct:
   - `grep "type C1Analyzer = c1.C1Analyzer" internal/analyzer/analyzer.go` confirms type alias
   - `grep "type C1Analyzer struct" internal/analyzer/c1_code_quality/codehealth.go` confirms type exists in moved file
...
`ls internal/analyzer/*.go` shows only analyzer.go and shared.go
  ...
  <done>
Full codebase builds successfully. All tests pass. No old analyzer files remain at root level. Type aliases in analyzer.go correctly reference types defined in moved files. Reorganization complete.
  </done>
</task>
...
4. `ls internal/analyzer/c1_code_quality/` shows: codehealth.go, python.go, typescript.go, and their tests (NO analyzer.go)
...
7. `grep "type C1Analyzer struct" internal/analyzer/c1_code_quality/codehealth.go` confirms type in moved file
</verification>
...
- Type definitions remain in moved files (codehealth.go has C1Analyzer, semantics.go has C2Analyzer, etc.)
- No separate analyzer.go files in subdirectories
