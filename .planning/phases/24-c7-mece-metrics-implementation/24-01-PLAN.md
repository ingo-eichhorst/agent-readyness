...
<task type="auto">
  <name>Task 1: Create Metric interface, common types, and registry with stubs</name>
  <files>
    internal/agent/metrics/metric.go
    internal/agent/metrics/registry.go
  </files>
  <action>
Create the `internal/agent/metrics/` package with the Metric interface AND stub constructors so the package compiles immediately.
...
func NewM1Consistency() Metric     { return &stubMetric{id: "task_execution_consistency", name: "Task Execution Consistency"} }
func NewM2Comprehension() Metric   { return &stubMetric{id: "code_behavior_comprehension", name: "Code Behavior Comprehension"} }
func NewM3Navigation() Metric      { return &stubMetric{id: "cross_file_navigation", name: "Cross-File Navigation"} }
func NewM4Identifiers() Metric     { return &stubMetric{id: "identifier_interpretability", name: "Identifier Interpretability"} }
func NewM5Documentation() Metric   { return &stubMetric{id: "documentation_accuracy_detection", name: "Documentation Accuracy Detection"} }
...
1. Package compiles immediately after Task 1
2. Registry can reference constructors
3. Task 2 replaces stub constructors with real implementations
  </action>
  <verify>
...
Registry returns 5: `go run -exec "" ./internal/agent/metrics 2>&1 | grep -c "stub"` or just compile check
  </verify>
  <done>
Metric interface defined with all required methods. MetricResult, Sample, SampleResult types defined. Registry compiles with stub implementations. Package builds successfully.
  </done>
</task>
...
- Has a constructor function that REPLACES the stub in metric.go
- Uses deterministic heuristic-based sample selection
- Has a specific prompt and rubric for Claude evaluation

**IMPORTANT: After creating each m*.go file, update metric.go to remove the corresponding stub constructor and import the real one. The final metric.go should have NO stub implementations.**

**Sample Selection Data Sources:**
The `AnalysisTarget` struct provides:
- `target.Files` - slice of `SourceFile` with `Path`, `RelPath`, `Lines`, `Content`, `Class`, `Language`
- `target.RootDir` - project root directory
- `target.Language` - language of this target

For complexity-based selection, you must:
1. Parse file content to count complexity heuristics (e.g., count if/for/switch statements, function definitions)
2. Use `file.Lines` as LOC proxy
3. Compute a selection score from these heuristics
...
- Sample selection: Pick 1 file with moderate lines (50-200 LOC) using `file.Lines`, then parse to find a function. Use heuristic: count `func ` occurrences in Content, prefer files with 3-10 functions.
...
- Sample selection: Parse each file's Content to count complexity indicators (if/for/switch/case statements). Compute score = complexity_count * (1/sqrt(Lines)). Sort descending, take top 3.
- Timeout: 120s
- Sample count: 3
...
- Sample selection: Parse Content to count import statements. Sort by import count descending, take top 2 non-test files (Class != ClassTest).
- Timeout: 120s
- Sample count: 2
...
- Sample selection: Extract exported identifiers (capitalized in Go, `export` in TS/JS) from Content. Sort by name length descending (longer = more semantic content), take top 5.
- Timeout: 60s
- Sample count: 5
...
- Sample selection: Parse Content to count comment lines (lines starting with `//` or within `/* */`). Compute comment density = comment_lines / total_lines. Sort by density descending, take top 3 files with density > 0.05.
- Timeout: 60s
- Sample count: 3
...
2. Constructor that sets defaults (REMOVE stub from metric.go)
3. SelectSamples() using deterministic heuristics - parsing file.Content as described above
...
No stubs remain: `grep -c "stubMetric" internal/agent/metrics/metric.go` should be 0
  </verify>
  <done>
All 5 MECE metrics implemented with deterministic sample selection (parsing file.Content for complexity/imports/comments), specific prompts, and scoring rubrics. Stub implementations removed from metric.go. Package compiles without errors. AllMetrics() returns all 5 real metrics.
  </done>
</task>
...
grep -c "stubMetric" internal/agent/metrics/metric.go || echo "No stubs found (good)"
```
</verification>
...
- No stub implementations remain in final code
</success_criteria>
