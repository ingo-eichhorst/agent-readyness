...
must_haves:
  truths:
    - "User sees all 5 metrics running simultaneously (parallel execution)"
    - "User can cancel evaluation and all metrics stop gracefully"
    - "User sees real-time progress updates during multi-metric execution"
    - "One failing metric does not prevent other metrics from completing"
  artifacts:
...
# Note: SUMMARY.md files do not exist yet - they are created after each plan executes
# The actual artifacts (metric.go, registry.go, progress.go) will exist from 24-01 and 24-02
...
Has fmt import: `grep '"fmt"' internal/agent/metrics/executor_adapter.go`
  </verify>
  <done>
CLIExecutorAdapter created that bridges metrics.Executor interface to real Claude CLI via agent.Executor. Includes all necessary imports (context, fmt, time, agent).
  </done>
</task>
...
# Imports verified
grep '"fmt"' internal/agent/metrics/executor_adapter.go
```
</verification>
