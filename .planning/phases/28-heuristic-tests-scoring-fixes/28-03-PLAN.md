...
must_haves:
  ...
  artifacts:
    ...
    - path: "internal/agent/metrics/m2_comprehension.go"
      provides: "Rebalanced scoring with grouped indicators (BaseScore=3, 4-6 thematic groups)"
      contains: "group:behavior_understanding"
    - path: "internal/agent/metrics/m3_navigation.go"
      provides: "Rebalanced scoring with grouped indicators (BaseScore=3, 4-6 thematic groups)"
      contains: "group:import_awareness"
    - path: "internal/agent/metrics/m4_identifiers.go"
      provides: "Rebalanced scoring with grouped indicators (BaseScore=3)"
      contains: "group:self_report_positive"
  key_links:
...
Purpose: Currently all M2/M3/M4 scoring functions over-saturate -- any decent response hits 10-15 positive indicators (each +1), pushing the score from BaseScore(5) to 15-20 before clamping to 10. This means zero differentiation between adequate and excellent responses. The fix groups related indicators into thematic categories (each group worth +1, fires if ANY member matches), reducing effective indicator count from ~20 to ~6 and lowering BaseScore to 3. This produces a usable scoring range of 3-9.
...
<task type="auto">
  <name>Task 2: Fix M2/M3/M4/M5 scoring saturation with grouped indicators</name>
  <files>
    internal/agent/metrics/m2_comprehension.go
    internal/agent/metrics/m3_navigation.go
    internal/agent/metrics/m4_identifiers.go
  ...
  <action>
    ...
    **Strategy: Group related indicators into thematic categories + lower BaseScore.**
    Each group fires +1 if ANY of its member indicators match. This reduces effective indicator count from ~20 individual +1s to ~6 grouped +1s. The IndicatorMatch.Delta type stays as int -- no type changes needed.

    - Change BaseScore from 5 to 3 for all scoring functions (gives room for upward movement)
    - Replace individual +1 indicators with grouped indicators (each group worth +1)
