# Agent Readiness Score - Development Guide

This document provides comprehensive guidance for AI agents and developers working with the Agent Readiness Score (ARS) codebase.
...
- **HTML templates are embedded at compile time** - rebuild the binary with `go build -o ars .` after changing template files in `internal/output/templates/`
